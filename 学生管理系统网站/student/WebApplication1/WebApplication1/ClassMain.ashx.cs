...
namespace WebApplication1
{
    ...
    /// ClassMain 的摘要说明
    /// </summary>
    public class ClassMain : IHttpHandler,IRequiresSessionState
    {

        public void ProcessRequest(HttpContext context)
        {
            context.Response.ContentType = "text/html";
            string classname = context.Request["Search"];
            //Session验证用户是否存在
            ...
            else
            {
                ...
                else
                {
                    ...
                    try
                    {
                        dt = SqlHelper.ExecuteDataTable("select * from class,teacher where classname like @classname and class.tid=teacher.tid", new SqlParameter("@classname", "%" + classname + "%"));
                    }
                    catch (Exception e) { }
                    var Data = new { Persons = dt.Rows };
                    string html = CommonHelper.RenderHtml("ClassMain.html", Data);
        ...
        private void ShowList(HttpContext context)
        {
            ...
            int totalCount = (int)SqlHelper.ExecuteScalar("select count(*) from class");
